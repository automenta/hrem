...
class Transformer(nn.Module):
    ...
    """
    def __init__(self, input_size: int, hidden_size: int, num_layers: int, num_heads: int, output_size: int, dropout: float = 0.1, **kwargs):
        """
        Initializes the Transformer model.

        Args:
            input_size (int): The number of input features per step.
            hidden_size (int): The number of features in the transformer layers (d_model).
            num_layers (int): The number of sub-encoder-layers in the encoder.
            num_heads (int): The number of heads in the multiheadattention models.
            output_size (int): The number of output features per step.
    ...
    def forward(self, batch: Dict[str, torch.Tensor]) -> Dict[str, torch.Tensor]:
        ...
        Args:
            batch (Dict[str, torch.Tensor]): A dictionary containing the input tensor
                                             under the key 'inputs' and target tensor
                                             under the key 'targets'.
        ...
        # Generate masks
        # The causal mask ensures that the prediction for position i can depend only on the known outputs at positions less than i.
