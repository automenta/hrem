...
class RotaryEmbedding(nn.Module):
    ...
    def _set_cos_sin_cache(self, seq_len, device):
        ...
        self.register_buffer("cos_cached", emb.cos()[None, None, :, :].to(self.dtype), persistent=False)
        self.register_buffer("sin_cached", emb.sin()[None, None, :, :].to(self.dtype), persistent=False)
...
class Attention(nn.Module):
    def __init__(self, hidden_size, num_heads, head_dim, causal=False, bias=False, dtype=torch.float32):
        super().__init__()
        self.num_heads = num_heads
        self.head_dim = head_dim
        self.causal = causal

        self.q_proj = nn.Linear(hidden_size, num_heads * head_dim, bias=bias, dtype=dtype)
        self.k_proj = nn.Linear(hidden_size, num_heads * head_dim, bias=bias, dtype=dtype)
        self.v_proj = nn.Linear(hidden_size, num_heads * head_dim, bias=bias, dtype=dtype)
        self.o_proj = nn.Linear(num_heads * head_dim, hidden_size, bias=bias, dtype=dtype)
...
class CastedLinear(nn.Linear):
    """
    An nn.Linear wrapper that casts the weights and bias to a specified dtype upon initialization.
    """
    def __init__(self, *args, cast_to: torch.dtype, **kwargs):
        super().__init__(*args, **kwargs)
        self.weight = nn.Parameter(self.weight.to(cast_to))
